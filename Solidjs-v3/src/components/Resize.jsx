...
export function SplitRoot(props) {
  ...
  return (
    <div
      class="split-root"
      data-split-resize-debounce={() => props.resizeDebounce || 0}
...
export function SplitY(props) {
  ...
  const getChildren = children(() => {
    ...
    const getChildArray = Array.isArray(props.children)
      ? props.children
      : [props.children];
    return getChildArray.map((getChild) => {
      ...
      const childComponent =
        getChild instanceof Function ? getChild() : getChild;
...
export function SplitItem(props) {
  ...
  function handleMoveStart(event) {
    ...
    const activeMoveParentClass =
      handleLeft || handleRight ? "split-horizontal" : "split-vertical";
    const sizeKeyPrefix = "offset"; // all the same?
    activeMoveSizeKey =
      activeMoveParentClass == "split-vertical"
    ...
    if (activeMoveParentClass == "split-vertical") {
      ...
      // TODO debounced resize: live move splitter + resize content every 100ms or so
        ...
        newStyle.marginTop =
          tar.offsetTop - event.clientY - handlePadding + "px";
      ...
      // TODO debounced resize: live move splitter + resize content every 100ms or so
        ...
        newStyle.marginLeft =
          tar.offsetLeft - event.clientX - handlePadding + "px";
  ...
  function calcLayout(event) {
    ...
    let node_size_new = containerChildren.map(
      (node) => node[activeMoveSizeKey]
    );
    ...
    if (
      ...
      for (let [index, node] of containerChildren.entries()) {
        ...
        if (node.parentElement.classList.contains("split-horizontal"))
          node.style.width = amount;
        else if (node.parentElement.classList.contains("split-vertical"))
          node.style.height = amount;
      }
    }
  }
